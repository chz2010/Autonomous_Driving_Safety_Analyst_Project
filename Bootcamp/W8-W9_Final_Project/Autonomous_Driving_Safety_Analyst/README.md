...
│
├── ingestion/
│   ├── video_ingestion.py       # YouTube → Chroma video DB
│   └── standards_ingestion.py  # ISO PDFs → Chroma standards DB
│
├── agent/                       # (next step) LangChain agent + tools
│
├── utils/
...
1. **Agent** (`agent/agent.py`) — LangChain agent with two retriever tools + safety lens synthesizer
2. **UI** (`app.py`) — Streamlit conversational interface with voice input (Whisper)
3. **LangSmith** — evaluation and tracing dashboard
