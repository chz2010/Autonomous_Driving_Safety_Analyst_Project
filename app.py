...
def sidebar_controls() -> tuple[str, bool, list[str], str]:
    ...
    with st.sidebar:
        ...
        selected_standards = [
            ...
            if st.checkbox(label, value=True, key=f"standard_{label}")
        ]
...
def render_scenario_form(
    ...
    if "scenario_intake_open" not in st.session_state:
        st.session_state.scenario_intake_open = False
    ...
    if st.button("Structured Scenario Intake", key="scenario_intake_toggle", use_container_width=True):
        st.session_state.scenario_intake_open = not st.session_state.scenario_intake_open

    if st.session_state.scenario_intake_open:
...
def render_speech_input(
    ...
    with st.popover("Voice", use_container_width=False):
        ...
        transcript = st.text_area(
            "Transcribed request",
            value=st.session_state.transcribed_prompt,
            height=110,
            placeholder="Your transcribed request will appear here. You can edit it before submitting.",
        )
        st.session_state.transcribed_prompt = transcript
...
def submit_prompt(
    ...
    """Submit prompt to the agent and update chat history."""
    if not prompt.strip():
        return
    if not selected_standards:
        st.warning("Select at least one standard before asking a question.")
